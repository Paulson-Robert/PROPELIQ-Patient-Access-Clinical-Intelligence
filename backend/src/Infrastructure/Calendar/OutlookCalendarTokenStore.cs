...
/// <summary>
/// Persists Microsoft Outlook Calendar OAuth tokens encrypted at rest using ASP.NET Core Data Protection.
/// Follows the same pattern as <see cref="CalendarTokenStore"/> for the Microsoft provider (AC-02).
/// Keys must be persisted to durable storage in production (e.g. Azure Key Vault, AWS SSM).
/// </summary>
public sealed class OutlookCalendarTokenStore : IOutlookCalendarTokenStore
{
    private const string PurposeName = "OutlookCalendarTokens.v1";

    private readonly ApplicationDbContext _db;
    private readonly IDataProtector _protector;

    public OutlookCalendarTokenStore(ApplicationDbContext db, IDataProtectionProvider dataProtectionProvider)
    {
        _db = db;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var existing = await _db.CalendarSyncs
            .FirstOrDefaultAsync(
                c => c.UserId == userId && c.Provider == CalendarProvider.Microsoft,
                cancellationToken)
        ...
        CancellationToken cancellationToken = default)
    {
        var sync = await _db.CalendarSyncs
            .AsNoTracking()
            .FirstOrDefaultAsync(
                c => c.UserId == userId && c.Provider == CalendarProvider.Microsoft && c.IsActive,
                cancellationToken)
            .ConfigureAwait(false);
